...
public static class DevInfoTools
{
    [McpServerTool(Name = "atlas_devinfo_list"),
     Description("프로젝트 개발 정보 (DevInfo) 조회 → JSON 배열")]
    ...
    [McpServerTool(Name = "atlas_devinfo_get"), Description("단일 개발 정보 조회")]
    ...
    [McpServerTool(Name = "atlas_devinfo_create"),
     Description("개발 정보 생성. type 은 Markdown|File|Link. Markdown 은 디스크에 .md 자동 export")]
    public static async Task<string> Create(
        DevInfoService svc,
        [Description("프로젝트 ID")] int projectId,
        [Description("제목")] string title,
        [Description("타입 Markdown|File|Link")] DevInfoType type,
        ...
        [Description("파일 경로 — File 타입")] string? filePath = null,
    ...
    [McpServerTool(Name = "atlas_devinfo_update"),
     Description("개발 정보 부분 갱신 — null 인 필드는 기존 값 유지")]
    ...
    [McpServerTool(Name = "atlas_devinfo_delete"),
     Description("개발 정보 삭제 (Markdown / Copy 모드는 디스크 파일도 함께 제거)")]
